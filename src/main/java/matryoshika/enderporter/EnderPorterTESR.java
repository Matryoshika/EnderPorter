...
public class EnderPorterTESR extends TileEntitySpecialRenderer<TileEnderPorter>{
    ...
    private IBakedModel getBakedModelBall() {
        ...
        if (bakedModelBall == null) {
            try {
                modelBall = ModelLoaderRegistry.getModel(new ResourceLocation(EnderPorter.MODID, "block/blockenderporter"));
            } catch (Exception e) {
    ...
    @Override
    public void render(TileEnderPorter te, double x, double y, double z, float partialTicks, int destroyStage, float unused){
    ...
    public void renderBody(TileEnderPorter te, double x, double y, double z, float ticks, int stage){
    	//GlStateManager.pushAttrib();
        GlStateManager.pushMatrix();
